(() => {
  ...
  function standingGroupLabel(group) {
    ...
    if (/^(east|central|west|north|south)$/i.test(name) && parent && !new RegExp(name, 'i').test(parent)) {
      return `${parent} ${name}`;
    }
    return name;
  }
  ...
  function renderRoster() {
    ...
    snapshot.roster.forEach(player => {
      ...
      if (player.position) identity.appendChild(el('div', 'roster-meta', player.position));
