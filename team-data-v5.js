(() => {
  ...
  function player(playerObj, groupName = '') {
    return {
      name: playerObj?.fullName || playerObj?.displayName || playerObj?.name || 'Unnamed player',
      position: playerObj?.position?.abbreviation || playerObj?.position?.displayName || playerObj?.position?.name || groupName || '',
